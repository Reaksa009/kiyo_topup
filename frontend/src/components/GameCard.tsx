...
export const GameCard: React.FC<GameCardProps> = ({ game }) => {
  ...
  return (
    <article className="group relative overflow-hidden rounded-3xl border border-white/[0.09] bg-white/[0.035] shadow-[0_18px_50px_rgba(0,0,0,0.18)] backdrop-blur-xl transition-transform duration-300 motion-safe:hover:-translate-y-1.5">
      <div className="relative aspect-[1.2/1] overflow-hidden bg-slate-900"><img src={image} alt={game.title} loading="lazy" decoding="async" sizes="(max-width: 640px) 50vw, (max-width: 1024px) 33vw, 25vw" className="h-full w-full object-cover transition duration-500 motion-safe:group-hover:scale-105" /><div className="absolute inset-0 bg-gradient-to-t from-[#0b1020] via-transparent to-transparent" /><div className="absolute left-3 top-3 flex flex-wrap gap-1.5"><span className="rounded-lg border border-white/10 bg-[#0b1020]/70 px-2 py-1 text-[9px] font-black uppercase tracking-wider text-cyan-100 backdrop-blur">{categoryName}</span>{game.discount && <span className="rounded-lg bg-emerald-300 px-2 py-1 text-[9px] font-black uppercase tracking-wider text-emerald-950">{game.discount}</span>}</div>{(game.isPopular || game.isFlashSale) && <span className="absolute bottom-3 left-3 inline-flex items-center gap-1 rounded-lg bg-violet-500/90 px-2 py-1 text-[9px] font-black uppercase tracking-wider text-white"><Flame className="h-3 w-3 text-amber-200" />{game.isFlashSale ? 'Flash deal' : 'Popular'}</span>}</div>
