...
export function GameDetail() {
  ...
  return (
    <div className="flex min-h-screen max-w-[100vw] flex-col overflow-x-hidden bg-[#fafafa] pb-24 text-slate-800 md:pb-0">
      ...
      <main className="min-w-0 flex-1">
        <section className="section-shell pt-3 sm:pt-5" aria-label="Game details banner">
          <div className="relative h-[290px] overflow-hidden rounded-[26px] bg-slate-950 shadow-[0_18px_50px_rgba(15,23,42,0.22)] sm:h-[380px] lg:h-[440px] lg:rounded-[32px]">
            <picture>
              <source media="(max-width: 639px)" srcSet={bannerMobile} />
              <img src={bannerDesktop} alt={`${game.title} banner`} fetchPriority="high" decoding="async" width="1200" height="440" loading="eager" className="h-full w-full object-cover" onError={(event) => { event.currentTarget.src = game.thumbnail; }} />
            </picture>
            <div className="absolute inset-0 bg-gradient-to-r from-slate-950/65 via-slate-950/5 to-transparent" />
            <div className="absolute inset-0 bg-gradient-to-t from-slate-950 via-slate-950/15 to-slate-950/20" />

            <Link to="/#games" className="game-detail-hero-back absolute left-4 top-4 inline-flex h-10 items-center gap-2 rounded-full border border-white/25 bg-slate-950/45 px-4 text-[10px] font-black text-white shadow-lg backdrop-blur-md transition hover:bg-slate-950/65 sm:left-6 sm:top-6">
              <ArrowLeft className="h-4 w-4" />{t('customer.backToGames')}
            </Link>

            <div className="absolute inset-x-0 bottom-0 flex items-end gap-3 p-4 sm:gap-4 sm:p-7 lg:p-9">
              <img src={game.thumbnail} alt="" decoding="async" width="88" height="88" loading="eager" className="h-16 w-16 shrink-0 rounded-2xl border-2 border-white/80 object-cover shadow-xl sm:h-20 sm:w-20" />
              <div className="game-detail-hero-copy min-w-0 pb-0.5 text-white">
                <p className="game-detail-hero-eyebrow mb-1 text-[9px] font-black uppercase tracking-[0.18em] text-amber-300 sm:text-[10px]">Instant game top-up</p>
                <h1 className="game-detail-hero-title line-clamp-2 text-xl font-black leading-tight tracking-tight sm:text-3xl lg:text-4xl">{game.title}</h1>
                <div className="game-detail-hero-meta mt-2 flex flex-wrap items-center gap-x-3 gap-y-1 text-[9px] font-bold text-white/75 sm:text-xs">
                  <span>{game.publisher || categoryName}</span>
                  <span className="game-detail-hero-rating inline-flex items-center gap-1 text-amber-300"><Star className="h-3.5 w-3.5 fill-amber-300" />4.9</span>
                  <span className="game-detail-hero-secure inline-flex items-center gap-1"><ShieldCheck className="h-3.5 w-3.5 text-emerald-300" />Secure payment</span>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="section-shell min-w-0 pb-8 pt-5 sm:pt-7">
        ...
        <div className="mt-4 grid min-w-0 grid-cols-1 gap-4 lg:grid-cols-3">
          <div className="min-w-0 space-y-4 lg:col-span-2">
            ...
            {/* Step 1: User ID */}
            <section className="relative min-w-0 overflow-hidden rounded-3xl border border-slate-200/80 bg-white p-5 shadow-sm sm:p-7">
              <div className="flex min-w-0 items-start gap-2.5 border-b border-slate-100 pb-3.5">
                ...
                <div className="min-w-0">
                  <h2 className="break-words text-sm font-black leading-snug text-slate-900 sm:text-base">បញ្ចូល ID របស់អ្នក / Enter User ID</h2>
                  <p className="mt-1 text-[10px] font-medium text-slate-500">Enter the account details below to continue.</p>
                </div>
              </div>
              ...
              <div className="mt-5 flex min-w-0 flex-col gap-4">
                {(game.inputFields || []).map((field) => {
                  ...
                  return (
                    <label key={field.name} className="block min-w-0 w-full">
                      ...
                      <input
                        ...
                        className="h-12 min-w-0 max-w-full w-full rounded-xl border border-slate-200 bg-slate-50 px-4 text-[16px] font-semibold text-slate-900 outline-none placeholder:font-medium placeholder:text-slate-400 focus:border-amber-400 focus:bg-white focus:ring-2 focus:ring-amber-400/15 md:text-sm"
                      />
