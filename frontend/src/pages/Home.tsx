  ...
  return (
    <div className="min-h-screen overflow-x-hidden bg-[#070a12] text-slate-100">
      ...
      <main>
        ...
        <section id="games" className="section-shell scroll-mt-24 py-14 sm:py-16 lg:py-20"><div className="flex flex-col gap-5 md:flex-row md:items-end md:justify-between"><div><span className="eyebrow">TOP-UP, YOUR WAY</span><h2 className="display-title mt-2">Popular games</h2><p className="mt-3 max-w-xl text-sm leading-6 text-slate-400">Everything you need to keep playing. Browse curated offers, compare package value and top up in a few simple steps.</p></div><div className="relative w-full md:w-72"><Search className="absolute left-3.5 top-3.5 h-4 w-4 text-slate-500" /><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search games" className="w-full rounded-2xl border bg-white/[0.04] py-3 pl-10 pr-4 text-xs text-white outline-none transition placeholder:text-slate-600 focus:border-cyan-300/50" /></div></div><div className="mt-6 flex gap-2 overflow-x-auto pb-1 sm:mt-7">{[{ slug: 'all', name: 'All games' }, ...categories].map((category) => <button key={category.slug} type="button" onClick={() => setSelectedCategory(category.slug)} className={`whitespace-nowrap rounded-xl border px-3.5 py-2 text-[10px] font-black transition sm:px-4 sm:text-[11px] ${selectedCategory === category.slug ? 'border-cyan-300/40 bg-cyan-300/[0.12] text-cyan-100' : 'border-white/[0.08] bg-white/[0.025] text-slate-400 hover:text-white'}`}>{category.name}</button>)}</div>{loading ? <div className="-mx-4 mt-6 flex snap-x snap-mandatory gap-3 overflow-x-auto px-4 pb-3 scroll-px-4 [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden sm:mx-0 sm:grid sm:grid-cols-3 sm:gap-4 sm:overflow-visible sm:px-0 sm:pb-0 lg:grid-cols-4">{Array.from({ length: 8 }).map((_, index) => <div key={index} className="h-52 w-[148px] shrink-0 snap-start animate-pulse rounded-[20px] bg-white/[0.04] sm:h-60 sm:w-auto sm:rounded-[22px]" />)}</div> : <div className="-mx-4 mt-6 flex snap-x snap-mandatory gap-3 overflow-x-auto px-4 pb-3 scroll-px-4 [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden sm:mx-0 sm:grid sm:grid-cols-3 sm:gap-4 sm:overflow-visible sm:px-0 sm:pb-0 lg:grid-cols-4">{filteredGames.map((game) => <GameCard key={game.slug} game={game} />)}</div>}{!loading && filteredGames.length === 0 && <div className="surface-card mt-8 py-16 text-center"><Search className="mx-auto h-8 w-8 text-slate-500" /><p className="mt-3 text-sm font-bold text-white">No games match your search</p><button type="button" onClick={() => { setSearch(''); setSelectedCategory('all'); }} className="mt-4 text-xs font-bold text-cyan-200 hover:text-white">Clear filters</button></div>}</section>
